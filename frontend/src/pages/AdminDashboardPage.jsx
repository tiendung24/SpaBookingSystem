    ...
    <AdminLayout>
      <header className="flex flex-col md:flex-row justify-between gap-4">
        <div>
          ...
          <p className="text-main/70">Theo dõi s?c kh?e h? th?ng, tang tru?ng d?i tác và v?n hành.</p>
          <AdminHeaderNav />
        </div>
        <div className="flex gap-2">
          <button className="px-5 py-3 rounded-xl bg-primary text-white font-bold shadow-lg hover:brightness-110" onClick={handleCreatePartner}>T?o d?i tác</button>
          <button className="px-5 py-3 rounded-xl bg-white border border-slate-200 text-main font-bold hover:bg-slate-50" onClick={handleExportReport}>Xu?t báo cáo</button>
        </div>
      </header>

      <section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
        <article className="glass-card bg-white rounded-3xl p-6">
          <p className="text-sm text-main/60">T?ng d?i tác</p>
          <p className="text-3xl font-bold text-primary mt-1">{stats.totalPartners}</p>
          <p className="text-sm text-main/60 mt-2">Ðang ho?t d?ng: <b className="text-emerald-600">{stats.activePartners}</b></p>
        </article>
        <article className="glass-card bg-white rounded-3xl p-6">
        ...
        <article className="glass-card bg-white rounded-3xl p-6">
          <p className="text-sm text-main/60">Ví LumiX (t?ng)</p>
      ...
      <section className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <article className="glass-card bg-white rounded-3xl p-6 space-y-3">
          <h4 className="font-h3 text-h3 text-primary">C?nh báo v?n hành</h4>
          <div className="p-4 rounded-2xl bg-amber-50 border border-amber-200 text-amber-900 text-sm">Có <b>{stats.pendingPartners}</b> d?i tác dang ch? duy?t / hoàn t?t onboarding.</div>
          <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 text-main/70 text-sm">G?i ý: theo dõi t? l? h?y/no-show b?t thu?ng và ngu?ng ví t?i thi?u.</div>
        </article>

        <article className="glass-card bg-white rounded-3xl p-6 xl:col-span-2 space-y-4">
          <div className="flex items-center justify-between">
            <h4 className="font-h3 text-h3 text-primary">Tình tr?ng h? th?ng</h4>
