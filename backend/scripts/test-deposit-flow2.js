...
async function main() {
  ...
  if (!service) {
    console.error('No services found in DB')
    await disconnectDb()
    process.exit(1)
  }
  const shop = await Shop.findById(String(service.shopId)).lean()
  if (!shop) {
    console.error('Shop for service not found', service.shopId)
    await disconnectDb()
    process.exit(1)
  }
  ...
  if (!depositAmount || depositAmount <= 0) {
    console.error('Shop does not require deposit or depositAmount is 0')
    await disconnectDb()
    process.exit(1)
  }
