...
export async function rebuildFakeBookings(req, res) {
  ...
  const schedule = [
    ...
    },
    {
      ...
      bookings: [
        ...
        { date: '2026-06-20', services: ['Nails tay', 'Nails tay'] },
        { date: '2026-06-22', services: ['Nails tay', 'Nails tay'] },
        { date: '2026-06-24', services: ['Nails tay'] },
        { date: '2026-06-26', services: ['Nails tay', 'Nails tay'] },
      ]
    },
    {
      ...
      bookings: [
        ...
        { date: '2026-06-27', services: ['Làm da', 'Làm da'] },
  ...
  for (const shopDef of schedule) {
    ...
    const findService = (hint) => {
      ...
      let matches = [];
      if (h.includes('gội')) matches = allServices.filter(s => s.name.toLowerCase().includes('gội'));
      else if (h.includes('mặt') || h.includes('da') || h.includes('mụn')) matches = allServices.filter(s => s.name.toLowerCase().includes('da') || s.name.toLowerCase().includes('thanh lọc'));
      else if (h.includes('nối mi')) matches = allServices.filter(s => s.name.toLowerCase().includes('mi'));
      else matches = allServices.filter(s => s.name.toLowerCase().includes('sửa form') || s.name.toLowerCase().includes('gel') || s.name.toLowerCase().includes('combo') || s.name.toLowerCase().includes('chà') || s.name.toLowerCase().includes('móng') || s.name.toLowerCase().includes('chân') || s.name.toLowerCase().includes('tay'));
      
      if (!matches.length) matches = allServices;
  ...
  res.json({ message: 'Hoàn tất sinh lại booking (random dịch vụ)', deleted: oldFakeBookingIds.length, totalAdded });
}
