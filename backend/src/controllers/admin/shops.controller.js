...
export async function syncFakeServices(req, res) {
  ...
  for (const shop of shops) {
    ...
    for (const b of bookings) {
      ...
      const numServices = Math.min(b.services.length, 3);
      const chosenServices = [];
      for(let i=0; i<numServices; i++) {
        const randS = insertedServices[Math.floor(Math.random() * insertedServices.length)];
        chosenServices.push({
          serviceId: String(randS._id),
          name: randS.name,
          duration: randS.duration,
          price: randS.price
        });
      }

      const totalAmount = chosenServices.reduce((sum, s) => sum + s.price, 0);
      const depositAmount = Math.min(Number(b.depositAmount || 0), totalAmount);
      const remainingAmount = totalAmount - depositAmount;

      b.services = chosenServices;
