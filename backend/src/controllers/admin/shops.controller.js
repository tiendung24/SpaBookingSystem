...
export async function sync50k(req, res) {
  const servicesUpdateResult = await Service.updateMany(
    { price: { $lt: 50000 } },
    { $set: { price: 50000, updatedAt: new Date() } }
  );

  const bookings = await Booking.find({ 'services.price': { $lt: 50000 } });
  ...
  for(const b of bookings) {
    ...
    if (isModified) {
      b.totalAmount = newTotalAmount;
      b.remainingAmount = b.totalAmount - (b.depositAmount || 0);
  ...
  res.json({
    message: 'Đồng bộ giá 50k thành công',
    servicesUpdated: servicesUpdateResult.modifiedCount,
    bookingsUpdated: updatedBookingsCount
  });
}
