...
export async function fixShopStaffs(req, res) {
  const shopData = [
  ...
  for (let i = 0; i < shopData.length; i++) {
    ...
    // Add 1 Normal Staff
    const normalStaff = new ShopStaff({
      shopId,
      fullName: 'Kỹ thuật viên',
      avatarUrl: genericAvatars[i % 2],
      role: 'staff',
      status: 'active',
      shortBio: 'Kỹ thuật viên chuyên nghiệp',
      bio: 'Tôi có nhiều kinh nghiệm trong việc chăm sóc và làm hài lòng khách hàng.',
      experienceYears: 3,
      specialties: ['Phục vụ tận tâm', 'Kỹ thuật tốt']
    });
  ...
  res.json({ message: 'Hoàn tất fix chủ shop', updated });
}
